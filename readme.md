...
`微软雅黑`、`微软雅黑 Bold`、`黑体`、`新宋体`、`华文琥珀` 五款字体随插件一起分发，开箱即用。

想换成别的字体，把字体文件放进 `data/p5-advance-letter-generator/fonts`，
文件名（去掉扩展名）就是配置项 `fonts` 里的字体族名，例如 `方正舒体.ttf`；
同名文件会覆盖内置的那份。放进去即时生效，不用重启。
